...
class PrometheusClient:
    ...
    async def _query_range_stats(self, query: str, time_range: str = "5m", start_time_iso: str = None) -> Dict[str, float]:
        ...
        try:
            ...
            if start_time_iso:
                ...
                logger.info(f"✓ Using custom start_time: {start_time_iso} -> {start_time}")
            else:
                start_time = end_time - 300  # 5 minutes
                logger.debug(f"Using default 5-minute window: {start_time}")
    ...
    async def query_host_metrics(self, time_range: str = "5m", start_time: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            cpu_stats = await self._query_range_stats(cpu_query, time_range, start_time)
            ...
            mem_stats = await self._query_range_stats(mem_query, time_range, start_time)
            
            # Disk read bytes/sec
            disk_read_query = f'sum(rate(system_disk_io_bytes_total{{direction="read",job="{job}"}}[1m]))'
            disk_read_stats = await self._query_range_stats(disk_read_query, time_range, start_time)
            
            # Disk write bytes/sec
            disk_write_query = f'sum(rate(system_disk_io_bytes_total{{direction="write",job="{job}"}}[1m]))'
            disk_write_stats = await self._query_range_stats(disk_write_query, time_range, start_time)
            
            # Network receive bytes/sec
            net_rx_query = f'sum(rate(system_network_io_bytes_total{{direction="receive",job="{job}"}}[1m]))'
            net_rx_stats = await self._query_range_stats(net_rx_query, time_range, start_time)
            
            # Network transmit bytes/sec
            net_tx_query = f'sum(rate(system_network_io_bytes_total{{direction="transmit",job="{job}"}}[1m]))'
            net_tx_stats = await self._query_range_stats(net_tx_query, time_range, start_time)
    ...
    async def query_otlp_metrics(self, time_range: str = "5m", start_time: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            http_total_stats = await self._query_range_stats(http_total_query, time_range, start_time)
            ...
            error_rate_stats = await self._query_range_stats(error_rate_query, time_range, start_time)
            
            # P95 latency from histogram
            p95_query = f'histogram_quantile(0.95, sum(http_server_duration_milliseconds_bucket{{job="{job}"}}) by (le))'
            p95_stats = await self._query_range_stats(p95_query, time_range, start_time)
            
            # P99 latency from histogram
            p99_query = f'histogram_quantile(0.99, sum(http_server_duration_milliseconds_bucket{{job="{job}"}}) by (le))'
            p99_stats = await self._query_range_stats(p99_query, time_range, start_time)
            
            # Active connections
            active_conn_query = f'http_server_active_requests{{job="{job}"}}'
            active_conn_stats = await self._query_range_stats(active_conn_query, time_range, start_time)
            
            # DB connections
            db_conn_query = f'db_client_connections_usage{{job="{job}"}}'
            db_conn_stats = await self._query_range_stats(db_conn_query, time_range, start_time)
            
            # Avg request size
            req_size_query = f'sum(http_server_request_size_bytes_sum{{job="{job}"}}) / sum(http_server_request_size_bytes_count{{job="{job}"}})'
            req_size_stats = await self._query_range_stats(req_size_query, time_range, start_time)
            
            # Avg response size
            resp_size_query = f'sum(http_server_response_size_bytes_sum{{job="{job}"}}) / sum(http_server_response_size_bytes_count{{job="{job}"}})'
            resp_size_stats = await self._query_range_stats(resp_size_query, time_range, start_time)
            ...
            avg_resp_time_stats = await self._query_range_stats(avg_resp_time_query, time_range, start_time)
            
            # P50 latency (median)
            p50_query = f'histogram_quantile(0.50, sum(http_server_duration_milliseconds_bucket{{job="{job}"}}) by (le))'
            p50_stats = await self._query_range_stats(p50_query, time_range, start_time)
            
            # P90 latency
            p90_query = f'histogram_quantile(0.90, sum(http_server_duration_milliseconds_bucket{{job="{job}"}}) by (le))'
            p90_stats = await self._query_range_stats(p90_query, time_range, start_time)
            
            # Request rate (req/sec) - using rate over time
            req_rate_query = f'sum(rate(http_server_duration_milliseconds_count{{job="{job}"}}[1m]))'
            req_rate_stats = await self._query_range_stats(req_rate_query, time_range, start_time)
            ...
            success_rate_stats = await self._query_range_stats(success_rate_query, time_range, start_time)
            
            # Asyncio process count
            asyncio_process_query = f'asyncio_process_created_total{{job="{job}"}}'
            asyncio_process_stats = await self._query_range_stats(asyncio_process_query, time_range, start_time)
            ...
            asyncio_duration_stats = await self._query_range_stats(asyncio_duration_query, time_range, start_time)
